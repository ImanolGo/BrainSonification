...
class MyFrame(wx.Frame):
    ...
    def init_panel(self):
        ...
        self.menubar.Append(play, '&Play')
        self.menubar.Append(view, '&View')
        self.menubar.Append(tools, '&Tools')
        self.menubar.Append(favorites, 'F&avorites')
    ...
    def init_data(self):
        ...
        f.close()    
        #self.num_figs = num.size(self.Data,0) -1 #sets the number of figures on the time axis
        self.num_figs = len(self.Data)
    def init_plot(self):
        """Draw data."""
        plt.jet()
        ...
        nrows_ncols = (6, 6),axes_pad = 0.2,share_all=True,
...
class MyApp(wx.App):
    ...
    def OnInit(self):
        self.frame = MyFrame(parent=None,id=-1, filename = self.filename)
